...
public class VanillaSession implements Session {
    final VanillaAsset root = new VanillaAsset(requestContext(""));
...
//        root.registerFactory(Subscription.class, f -> (Subscription) f.parent().acquireView(SubscriptionKeyValueStore.class, f.queryString()));
    ...
    public <A> Asset acquireAsset(RequestContext request) throws AssetNotFoundException {
        return request.name().isEmpty() ? root : root.acquireChild(request.fullName(), request.assetType(), request.type(), request.type2());
    }
