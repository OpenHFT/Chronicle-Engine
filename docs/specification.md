...
|get<key>  | returns non null |[link](https://github.com/OpenHFT/Chronicle-Engine/blob/master/docs/examples/get-returns-a-value.md)   |
...
this name maps to a channel id. this channel id must be uniquely mapped to the service name.
...
5. Data chunking, certain methods like map.put(), map.entry(), map.keySet() map.values() can cause large messages to be sent. Although large messages are often the most efficient way to send a lot of data this can be detrimental to other services ( especially services running on the same client machine, but large messages can even effect other services on your network. As such, it makes sense to chunk large messages ( and throttle them where appripiate ). Chronicle Engine Map, supports either sending all the entries as a single message or chunking them down to even individual entries, this chunk size is determined by your preferred TCP buffer size, when the buffer is about 50% full the code will opt to send the rest of the data in another chunk.
